...
type
  TfrmRetiradaR = class(TfrmRelatorio)
    QRLabel1: TQRLabel;
    lblData: TQRLabel;
    QRGroup1: TQRGroup;
    QRBand1: TQRBand;
...
var
  frmRetiradaR: TfrmRetiradaR;
  Total, TotalGeral : Double;
implementation
uses Main, Principal;
{$R *.dfm}

procedure TfrmRetiradaR.bndTituloBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TfrmRetiradaR.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  Total := 0;
  TotalGeral := 0;
...
procedure TfrmRetiradaR.Relatorio;
begin
...
procedure TfrmRetiradaR.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  edtData.Caption := DateToStr(qrRelatorio.FieldByname('CADASTRO').AsDateTime);
end;

procedure TfrmRetiradaR.QRDBText4Print(sender: TObject; var Value: String);
begin
  Value := FormatFloat('#,##0.00', qrRElatorio.FieldByname('VALOR').asFloat);
end;

procedure TfrmRetiradaR.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TfrmRetiradaR.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TfrmRetiradaR.lalalBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
